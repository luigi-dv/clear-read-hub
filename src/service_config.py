...
class ServiceConfig(BaseSettings, object):
    ...
    SERVICE_OAUTH_CLIENT_SECRET: str = Field(default='', env='SERVICE_OAUTH_CLIENT_SECRET')
    SERVICE_OAUTH_ALGORITHM: str = Field(default='HS256', env='SERVICE_OAUTH_ALGORITHM')
    SERVICE_OAUTH_EXPIRES_IN: int = Field(default=timedelta(minutes=15), env='SEVICE_OAUTH_EXPIRES_IN')
    # MONGODB
    MONGO_INITDB_ROOT_HOST: str = Field(default='localhost', env='MONGO_INITDB_ROOT_HOST')
    MONGO_INITDB_ROOT_PORT: int = Field(default=27017, env='MONGO_INITDB_ROOT_PORT')
    MONGO_INITDB_ROOT_USERNAME: str = Field(default='clearreadhub', env='MONGO_INITDB_ROOT_USERNAME')
    MONGO_INITDB_ROOT_PASSWORD: str = Field(default='root', env='MONGO_INITDB_ROOT_PASSWORD')
    ...
    def generate_connection_string(cls, v, values):
        ...
        az_storage_account_name = values.data[
            'AZ_STORAGE_ACCOUNT_NAME'] if 'AZ_STORAGE_ACCOUNT_NAME' in values.data else None
        az_storage_account_key = values.data[
            'AZ_STORAGE_ACCOUNT_KEY'] if 'AZ_STORAGE_ACCOUNT_KEY' in values.data else None
        az_storage_blob_endpoint = values.data[
            'AZ_STORAGE_BLOB_ENDPOINT'] if 'AZ_STORAGE_BLOB_ENDPOINT' in values.data else None
    ...
    def generate_mongo_connection_string(cls, v, values):
        mongo_initdb_root_host = values.data[
            'MONGO_INITDB_ROOT_HOST'] if 'MONGO_INITDB_ROOT_HOST' in values.data else None
        mongo_initdb_root_port = values.data[
            'MONGO_INITDB_ROOT_PORT'] if 'MONGO_INITDB_ROOT_PORT' in values.data else None
        mongo_initdb_root_username = values.data[
            'MONGO_INITDB_ROOT_USERNAME'] if 'MONGO_INITDB_ROOT_USERNAME' in values.data else None
        mongo_initdb_root_password = values.data[
            'MONGO_INITDB_ROOT_PASSWORD'] if 'MONGO_INITDB_ROOT_PASSWORD' in values.data else None

        if all(item is not None for item in [mongo_initdb_root_username, mongo_initdb_root_password]):
